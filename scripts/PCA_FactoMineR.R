...
pca <- PCA(data[,1:ncol(data)], scale.unit = TRUE, ncp = 5, graph = FALSE)

PC1 = pca$ind$coord[,1]
PC2 = pca$ind$coord[,2]
write.csv(pca$ind$coord,file="results\\PCA_coordinates_FactoMineR.csv")
...
plotdata = data.frame(rownames(pca$ind$coord), PC1, PC2, groups$group)
...
ggsave("results\\PCA_FactoMineR.pdf", device = cairo_pdf, height = 10, width = 12)

png(filename = "results\\PCA_FactoMineR.png", res = 600, height = 5400, width = 7200, type = "cairo")
print(plot)
dev.off()
