...
## p value ------------------------
anosim_result_dis <- anosim(distance, grouping = groups$group, permutations = 999)

R = anosim_result_dis$statistic
pvalue = anosim_result_dis$signif

anosim <- paste("ANOSIM:\nR = ", round(R,4), "\nP-value = ", pvalue)


## NMDS plot + boxplot using ggplot2 ------------------------
...
cairo_pdf(filename = "results\\NMDS_box_dot_combine.pdf", height = 8, width = 8)
p
dev.off()

png(filename = "results\\NMDS_box_dot_combine.png",res = 600, height = 5400, width = 5400, type = "cairo")
p
dev.off()
